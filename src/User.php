...
class User {
	 ...
	 * @param bool $get_username_remote Whether to get the username from the remote URL
	 * @return User self
	 */
	public static function fromJson($json, $get_username_remote = false) {
		...
		error_log('User::fromJson: '.var_export($json->data->user, 1));
		...
		$handle = '';
		if (!empty($json->data->user->username)) {
